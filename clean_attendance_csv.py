...
for file_name in os.listdir('.'):
    ...
    if file_name.endswith('.csv'):
        ...
        with open(file_name, 'r') as csv_file:
            ...
            cleaned_rows.append(["ID", "Name", "Days Enrolled", "Days Not Enrolled",
                                "Days Present", "Days Excused", "Days Not Excused"])
            ...
            for row in reader:
                ...
                if row and row[0].isdigit():
                    ...
                    if row[3]:
                        ...
                        cleaned_row = [row[0], row[3], row[9],
                                       row[11], row[12], row[14], row[15]]
                    else:
                        # Check if the 22nd cell is blank
                        if row[21]:
                            # Keep only the specified cells from the row
                            cleaned_row = [row[0], row[5], row[16],
                                           row[18], row[21], row[23], row[24]]
                        else:
                            # Keep only the specified cells from the row
                            cleaned_row = [row[0], row[5], row[16],
                                           row[18], row[23], row[25], row[26]]
